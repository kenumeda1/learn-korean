...
- **Who it's for:** English-speaking learners of Korean who want structured feedback and vocabulary tools in one calm workspace. (Stated in the hero one-liner; UI copy and prompts stay English-forward.)
...
- **Buttons:** `filter: brightness(1.05)` on hover for solid primaries (e.g. `.add-btn`); `.generate-btn` is soft secondary (`--accent-soft` fill, `--pill-noun-text` label) with a slightly deeper blue hover; active `scale(0.99)` on `.generate-btn`.
- **Transitions:** ~**0.15s** on backgrounds, borders, color for filters and icon buttons.

## Components (naming)

Key classes documented in CSS: `.hero-one-liner`, `.how-it-works`, `.hero-how`, `.input-shell`, `.add-btn` (solid primary), `.generate-btn` (soft secondary), `.filters`, `.card`, `.tag` (+ `--noun` / `--verb` / `--adjective` / `--sentence`), `.icon-btn`, `.progress`, `.alert-error`, `.history-`*, `.settings-details`, `.field-input`.

**Hero / density:** “How this works” lives in `<details class="how-it-works">`. First visit opens it; `localStorage` key `lh_hero_returning` is set when the user collapses it or completes a successful **Generate sentence**, so later visits start with it closed.
...
| 2026-04-11 | Hero: collapsible “How this works”             | Reduces density; key `lh_hero_returning` after collapse or successful generate.                 |
